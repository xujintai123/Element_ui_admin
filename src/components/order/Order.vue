<template>
  <div id="order">
    ...
    <el-card>
      ...
      <!-- 页码区域 -->
      <el-pagination @size-change="pageSizeChange" @current-change="pageCurrentChange" :current-page="queryInfo.pagenum"
        :page-sizes="[10, 15, 30]" :page-size="queryInfo.pagesize" layout="total, sizes, prev, pager, next, jumper"
        :total="total" background></el-pagination>

      <!-- 编辑地址对话框 -->
      <el-dialog title="修改地址" :visible.sync="editAdressVisible" width="50%" @close="addressFormClose">
        <!-- 表单区域 -->
        <el-form :model="addressForm" :rules="addressFormRules" ref="editorAddressForm" label-width="100px">
          <el-form-item label="省市区/县" prop="address1">
            <!-- 省市区/县级联选择器 -->
            <el-cascader v-model="addressForm.address1" :options="regionData" :props="{ expandTrigger: 'hover' }"
              @change="address1Change"></el-cascader>
          </el-form-item>
      ...
      <el-dialog title="物流进度" :visible.sync="getAdressVisible" width="50%">
        ...
        <el-timeline :reverse="false">
          <el-timeline-item v-for="(item, index) in addressInfo" :key="index" :timestamp="item.time">{{item.context}}
          </el-timeline-item>
        </el-timeline>
      </el-dialog>
      <!-- 卡片底部区域 -->

<template>
  <div id="pramas">
    ...
    <el-card>
      <!-- 警告区域 -->
      <el-alert :closable="false" center show-icon title="注意：只允许为第三级分类设置相关参数！" type="warning"></el-alert>
          ...
          <el-table border :data="manyTableData" stripe style="width: 100%">
            ...
            <el-table-column type="expand">
              ...
              <template slot-scope="scope">
                <el-tag v-for="(item,index) in scope.row.attr_vals" :key="index" closable
                  @close="removeParamsTag(index,scope.row)">{{item}}</el-tag>
                <!-- 添加新Tag的输入文本框 -->
                <el-input class="addTagInput" v-if="scope.row.addTagVisible" v-model="scope.row.addTagInputValue"
                  ref="addTagInput" size="small" @keyup.enter.native="addTagInputConfirm(scope.row)"
                  @blur="addTagInputConfirm(scope.row)"></el-input>
                <!-- 添加新Tag按钮 -->
                <el-button v-else class="addTagButton" size="small" @click="showAddTagInput(scope.row)">+ New Tag
                </el-button>
              </template>
            </el-table-column>
            <!-- 索引列 -->
            ...
            <el-table-column align="center" label="操作" width="600">
              ...
              <template slot-scope="scope">
                ...
                <el-button @click="deleteParamsData(scope.row.attr_id)" size="mini" type="danger" icon="el-icon-delete">
                  删除</el-button>
              </template>
            </el-table-column>
          </el-table>
          ...
          <el-table border :data="onlyTableData" stripe style="width: 100%">
            ...
            <el-table-column align="center" label="操作" width="600">
              ...
              <template slot-scope="scope">
                ...
                <el-button size="mini" type="danger" @click="deleteParamsData(scope.row.attr_id)" icon="el-icon-delete">
                  删除</el-button>
              </template>
            </el-table-column>
          </el-table>
    ...
    <el-dialog @close="addParamsFormClose" :title="'添加'+FormDataType" :visible.sync="addParamsFormVisible" width="50%">
      <!-- 验证表单数据 -->
    ...
    <el-dialog @close="editParamsFormClose" :title="'添加'+FormDataType" :visible.sync="editParamsFormVisible"
      width="50%">
      <!-- 验证表单数据 -->
      <el-form :model="editParamsForm" :rules="editParamsFormRules" ref="editParamsForm" label-width="100px">
        <el-form-item :label="FormDataType" prop="attr_name">
  ...
  export default {
    ...
    methods: {
      ...
      async getParmsData() {
        ...
        const {
          data: res
        } = await this.$http.get(`categories/${this.selectedCateKeys[2]}/attributes`, {
          params: {
      ...
      addParamsFormData() {
        this.$refs.addParamsForm.validate(async valid => {
          ...
          const {
            data: res
          } = await this.$http.post(`categories/${this.selectedCateKeys[2]}/attributes`, {
      ...
      async showEditDialog(Id) {
        ...
        const {
          data: res
        } = await this.$http.get(`categories/${this.selectedCateKeys[2]}/attributes/${Id}`, {
          attr_sel: this.activeTabName
        })
      ...
      editParamsFormData() {
        this.$refs.editParamsForm.validate(async valid => {
          ...
          const {
            ...
            `categories/${this.selectedCateKeys[2]}/attributes/${this.editParamsForm.attr_id}`, {
      ...
      async deleteParamsData(Id) {
        ...
        const {
          data: res
        } = await this.$http.delete(`categories/${this.selectedCateKeys[2]}/attributes/${Id}`)
        if (res.meta.status !== 200)
      ...
      async saveAddTagInput(row) {
        const {
          data: res
        } = await this.$http.put(`categories/${this.selectedCateKeys[2]}/attributes/${row.attr_id}`, {

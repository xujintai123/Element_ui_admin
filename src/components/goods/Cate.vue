<template>
  <div id="cate">
    ...
    <el-card>
      ...
      <table-tree class="table-tree" index-text="#" :expand-type="false" :selection-type="false" align="center"
        headerAlign="center" show-index border stripe :data="cateList" :columns="columns">
        <!-- 是否有效 -->
        ...
        <template slot="option" slot-scope="scope">
          <el-button size="mini" type="primary" icon="el-icon-edit" @click="editCateName(scope.row.cat_id)">编辑
          </el-button>
          <el-button size="mini" type="danger" icon="el-icon-delete" @click="deleteCateName(scope.row.cat_id)">删除
          </el-button>
        </template>
      </table-tree>
      ...
      <el-form :model="addCateForm" ref="addCateFormRef" label-width="100px" :rules="addCateFormRules">
        ...
        <el-form-item label="父级分类:" prop="cat_name">
          ...
          <el-cascader clearable v-model="selectedKeys" :options="parentCateList"
            ...
            @change="parentCateChange" expandTrigger="hover"></el-cascader>
        </el-form-item>
      </el-form>
  ...
  export default {
    ...
    methods: {
      ...
      editCateName(editId) {
        ...
        console.log(this.editCateForm.id = editId);
      },
      //监听编辑分类的对话框关闭事件,清空对话框数据
      editCateDialogClose() {
        this.editCateForm.cat_name = ''
      },
      //点击编辑按钮，提交数据发送请求，编辑分类名称
      editCateFormData() {
        this.$refs.editCateFormRef.validate(async validate => {
          if (validate) {
            const {
              data: res
            } = await this.$http.put(`categories/${this.editCateForm.id}`, this.editCateForm)
            if (res.meta.status !== 200)
              return this.$message.error('服务器繁忙,数据请求失败')

            this.$message.success('商品分类名称修改成功')
            //重新请求数据,刷新页面
            this.getCateList()
            this.editCateDialogVisible = false
          } else {
            this.$message.error('填写信息不符合规范，请重新填写')
          }
        })
      },
      //点击删除按钮，提交数据发送请求，删除此分类
      async deleteCateName(deleteId) {
        const result = await this.$confirm('此操作将永久删除该文件, 是否继续?',
          '提示', {
        ...
        if (result === "cancel")
          return this.$message.info('取消删除此分类')

        const {
          data: res
        } = await this.$http.delete(`categories/${deleteId}`, {
          id: deleteId
        })

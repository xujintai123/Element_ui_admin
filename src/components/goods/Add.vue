<template>
  <div id="add">
    ...
    <el-card>
      ...
      <!-- 侧边标签页区域 -->
      <!-- 侧边栏表单 -->
      <el-form label-position="top" :model="goodsAddForm" :rules="goodsAddRules" ref="goodsAddForm" label-width="100px"
        class="goodsAddForm">
        <!-- 侧边栏 -->
        <el-tabs v-model="activeSteps" :tab-position="'left'" :before-leave="beforeTabLeave">
          <el-tab-pane name="0" label="基本信息">
            ...
            <el-form-item label="商品分类" prop="goods_cat">
              <!-- 复选框开始 -->
              <el-cascader @change="selectedCateChange" v-model="goodsAddForm.goods_cat" :options="cateList"
                :props="{ expandTrigger:'hover',children:'children',value:'cat_id',label:'cat_name'}"></el-cascader>
            </el-form-item>
          </el-tab-pane>
          ...
          <el-tab-pane name="3" label="商品图片">
            ...
            <!-- on-remove处理移除图片操作 -->
            <!-- headers为请求头对象 -->
            <el-upload :headers="headerObj" :action="uploadURL" :on-preview="imgPreview" :on-remove="imgRemove"
              :on-success="imgSuccess" list-type="picture">
  ...
  export default {
    ...
    methods: {
      ...
      async getCateList() {
        const {
          data: res
        } = await this.$http.get('categories', this.query)
      ...
      async getCateParams(sel) {
        const {
          data: res
        } = await this.$http.get(`categories/${this.goodsAddForm.goods_cat[2]}/attributes`, {
          params: {
            sel: sel
          }
        })

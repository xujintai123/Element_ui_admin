<template>
  <div id="roles">
      ...
      <el-table :data="roleList" stripe style="width: 100%">
        ...
        <el-table-column type="expand" class="expand-table">
          <template slot-scope="scope">
            <el-row :class="['bd-bottom',index===0?'bd-top':'']" :gutter="20"
              ...
              <el-col :span="18">
                <el-row :class="[index!==0?'bd-top':'']" v-for="(item2,index) in item1.children" :key="item2.id">
                  <el-col :span="8">
                    <!-- 通过for循环渲染二级权限 -->
                    <el-tag @close="removeRightById(item2.id,scope.row)" closable type="success">{{item2.authName}}
                    </el-tag>
                    <i class="el-icon-caret-right"></i>
                  </el-col>
                  <el-col :span="12">
                    <!-- 通过for循环渲染三级权限 -->
                    <el-tag @close="removeRightById(item3.id,scope.row)" closable type="warning"
        ...
        <el-table-column prop="address" label="操作" align="center">
          <template slot-scope="scope">
            ...
            <el-button @click="showSetRightDialog(scope.row)" type="warning" icon="el-icon-share" size="mini">分配权限
            </el-button>
          </template>
        </el-table-column>
      </el-table>
    ...
    <el-dialog title="分配权限" :visible.sync=" SetRightDialogVisible" width="50%">
      <!-- 树形控件 -->
      <el-tree ref="treeRef" :default-checked-keys="defKeys" :data="rightsList" :props="treeProps" show-checkbox
